...
async def test_cycle_plan_pra(client_org):
    r = await client_org.post("/v1/pra", json=_pra())
    assert r.status_code == 201, r.text
    plan = r.json()
    assert plan["nom"] == "pra-prod" and plan["statut"] == "jamais_teste"
    pra_id = plan["id"]

    r = await client_org.get(f"/v1/pra/{pra_id}")
    assert r.status_code == 200 and r.json()["exercices"] == []

    r = await client_org.patch(f"/v1/pra/{pra_id}", json={**_pra(), "nom": "pra-prod-v2"})
    assert r.status_code == 200 and r.json()["nom"] == "pra-prod-v2"

    r = await client_org.delete(f"/v1/pra/{pra_id}", params={"confirmation": "pra-prod-v2"})
    assert r.status_code == 204


async def test_modifier_plan_pra_sans_dependances(client_org):
    """Régression : `dependances` est optionnel sur `GroupePra` (le contrat de création), mais
    requis sur le `Groupe` stocké — un client_org qui l'omet sur un PATCH (comme le fait le
...
async def test_replication_continue_refusee(client_org):
...
async def test_bascule_test_et_exercice(client_org):
    r = await client_org.post("/v1/pra", json=_pra())
    pra_id = r.json()["id"]

    r = await client_org.post(f"/v1/pra/{pra_id}/bascule", json={"type": "test"})
...
async def test_bascule_reelle_avec_confirmation(client_org):
    r = await client_org.post("/v1/pra", json=_pra())
    pra_id = r.json()["id"]

    r = await client_org.post(f"/v1/pra/{pra_id}/bascule", json={"type": "reel"})
...
async def test_retour_site_source(client_org):
    ...
    r = await client_org.post(f"/v1/pra/{pra_id}/retour", json={"confirmation": "pra-prod"})
    assert r.status_code == 202, r.text
