...
async def test_cycle_relais(client_org):
    r = await client_org.get("/v1/web/smtp")
    ...
    relais = (await client_org.get("/v1/web/smtp")).json()
    assert relais["actif"] is True
    assert relais["hote"] == "smtp.synelia.cloud"
    assert 587 in relais["ports"]

    r = await client_org.post("/v1/web/smtp", json={"domainesAutorises": ["autre.ci"]})
    assert r.status_code == 409 and r.json()["erreur"]["code"] == "relais_deja_actif"

    r = await client_org.patch("/v1/web/smtp", json={"domainesAutorises": ["exemple.ci", "b.ci"]})
    ...
    r = await client_org.patch("/v1/web/smtp", json={"quotaJour": 4000})
    ...
    r = await client_org.get("/v1/web/smtp/messages")
    assert r.status_code == 200 and r.json()["donnees"] == []


async def test_cles_smtp(client_org):
    await client_org.post("/v1/web/smtp", json={"domainesAutorises": ["exemple.ci"]})
    r = await client_org.post("/v1/web/smtp/cles", json={"nom": "app-prod", "quotaJour": 200})
    ...
    r = await client_org.patch(f"/v1/web/smtp/cles/{cid}", json={"quotaJour": 300})
    assert r.status_code == 200 and r.json()["quotaJour"] == 300

    r = await client_org.delete(f"/v1/web/smtp/cles/{cid}", params={"confirmation": "mauvais"})
    assert r.status_code == 422
    r = await client_org.delete(f"/v1/web/smtp/cles/{cid}", params={"confirmation": "app-prod"})
    assert r.status_code == 204
    cles = (await client_org.get("/v1/web/smtp/cles")).json()
    assert cles[0]["statut"] == "revoquee"


async def test_identifiants_regenerer(client_org):
    await client_org.post("/v1/web/smtp", json={"domainesAutorises": ["exemple.ci"]})
    r = await client_org.post("/v1/web/smtp/identifiants", json={"confirmation": "regenerer"})
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["motDePasse"] and body["identifiant"]


async def test_webhooks_smtp(client_org):
    await client_org.post("/v1/web/smtp", json={"domainesAutorises": ["exemple.ci"]})
    r = await client_org.post(
    ...
    r = await client_org.post(f"/v1/web/smtp/webhooks/{wid}/test")
    assert r.status_code == 200 and r.json()["envoye"] is True

    r = await client_org.delete(f"/v1/web/smtp/webhooks/{wid}")
    assert r.status_code == 204
    assert (await client_org.get("/v1/web/smtp/webhooks")).json() == []


async def test_erreurs_smtp(client_org):
    ...
    r = await client_org.patch("/v1/web/smtp", json={"domainesAutorises": ["x.ci"]})
    ...
    await client_org.post("/v1/web/smtp", json={"domainesAutorises": ["exemple.ci"]})
    r = await client_org.post("/v1/web/smtp/cles", json={"nom": "ci", "quotaJour": 10})
    assert r.status_code == 201, r.text
    cid = r.json()["cle"]["id"]
    r = await client_org.delete(f"/v1/web/smtp/cles/{cid}", params={"confirmation": "mauvais"})
    assert r.status_code == 422
    r = await client_org.get("/v1/web/smtp/cles", params={"statut": "active"})
