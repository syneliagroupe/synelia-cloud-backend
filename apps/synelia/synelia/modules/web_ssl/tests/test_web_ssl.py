...
async def test_offres_certificat(client_org):
    r = await client_org.get("/v1/web/ssl/offres")
...
async def test_cycle_certificat(client_org):
    ...
    r = await client_org.get("/v1/web/ssl")
    assert r.status_code == 200
    certifs = r.json()["donnees"]
    assert len(certifs) == 1 and certifs[0]["etat"] == "actif"
    cid = certifs[0]["id"]

    r = await client_org.post(f"/v1/web/ssl/{cid}/validation")
    assert r.status_code == 409 and r.json()["erreur"]["code"] == "certificat_deja_valide"

    r = await client_org.patch(f"/v1/web/ssl/{cid}", json={"renouvellementAuto": False})
    assert r.status_code == 200 and r.json()["renouvellementAuto"] is False

    r = await client_org.post(f"/v1/web/ssl/{cid}/renouvellement", json={"dureeAnnees": 1})
    assert r.status_code == 202, r.text
    assert r.json()["type"] == "web.ssl.renew" and r.json()["statut"] == "done"
    cert = (await client_org.get(f"/v1/web/ssl/{cid}")).json()
    assert cert["etat"] == "actif"

    r = await client_org.delete(f"/v1/web/ssl/{cid}", params={"confirmation": "mauvais"})
    assert r.status_code == 422
    r = await client_org.delete(f"/v1/web/ssl/{cid}", params={"confirmation": "www.ci"})
    assert r.status_code == 204
    cert = (await client_org.get(f"/v1/web/ssl/{cid}")).json()
    assert cert["etat"] == "revoque"


async def test_commande_le_expire_90j(client_org):
    r = await client_org.post(
        "/v1/web/ssl", json={"hote": "le.ci", "type": "letsencrypt", "validationDomaine": "http"}
    )
    assert r.status_code == 202
    cert = next(
        c for c in (await client_org.get("/v1/web/ssl")).json()["donnees"] if c["hote"] == "le.ci"
    )
...
async def test_erreurs_ssl(client_org):
    ...
    r = await client_org.post("/v1/web/ssl", json=corps)
    assert r.status_code == 202, r.text
    r = await client_org.post("/v1/web/ssl", json=corps)
    assert r.status_code == 409

    r = await client_org.get("/v1/web/ssl", params={"etat": "actif"})
    assert r.status_code == 200
