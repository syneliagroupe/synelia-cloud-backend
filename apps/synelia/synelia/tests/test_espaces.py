...
async def test_cycle_espace(client_org):
    ...
    r = await client_org.post("/v1/espaces", json=corps)
    ...
    r = await client_org.get(f"/v1/travaux/{travail['id']}")
    assert r.status_code == 200 and r.json()["statut"] == "done"

    r = await client_org.post("/v1/espaces", json=corps)
    assert r.status_code == 409 and r.json()["erreur"]["code"] == "nom_deja_pris"

    r = await client_org.delete(f"/v1/espaces/{eid}", params={"confirmation": "mauvais"})
