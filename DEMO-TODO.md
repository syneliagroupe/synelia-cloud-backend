...
> ⚠️ **MISE À JOUR (2026-09-15, ~16h50) : `comp1` est RÉPARÉ, `comp2` reste cassé.**
> Root cause enfin trouvée (voir l'entrée « Résolu » correspondante, section
> « ROOT CAUSE ENFIN TROUVÉE ») : `ctrl1` était en épuisement mémoire complet (swap
> plein), pas un bug RabbitMQ/heartbeat. VM `ctrl1` augmentée 28→40 Go, `comp1` de
> nouveau `state: up` et stable — la création de VM/hébergement/cluster fonctionne
> à nouveau **si le scheduler Nova place la ressource sur `comp1`**. `comp2` est
> toujours `state: down` (cause différente : queues RabbitMQ fanout gonflées à
> 20 000+ messages non consommés, RabbitMQ nack les réponses RPC — pas encore
> réglé, nécessite une purge spécifique aux stream queues). Si une création échoue
> encore, vérifiez d'abord si le lab a d'autres soucis nouveaux (l'épuisement
> réseau Neutron peut revenir si beaucoup de tests tournent en parallèle sans
> nettoyage — voir l'entrée dédiée) avant de re-diagnostiquer RabbitMQ depuis zéro.
>
...
Dernière mise à jour : 2026-09-15 (après 18h46, VM test infra fixes including image catalogue API response fix).
**VM test infrastructure fixes (2026-09-15, 18h42-46)** : commit e615503 updated test_vms.py to fetch real image IDs from `GET /catalogue/images` instead of hardcoded names (`ubuntu-24.04`, `debian-12`), mirroring the `_gabarit_id()` pattern. Now that comp1 is stable again (ctrl1 memory 28→40 Go), VM creation tests can use real OpenStack resources. Commit e615503 happy-path tests updated: `_creer_vm`, `test_creer_vm_explicite_et_image_inconnue`, `test_creer_vm_nom_deja_pris`, `test_lot_vms`; negative tests kept literal invalid strings. Initial test run revealed **API response format mismatch bug**: `_image_id()` was parsing `r.json()["donnees"]` but `GET /catalogue/images` returns a list directly, not a wrapped dict. Fixed in commit 8706d60 (18h46): `images = r.json()` without `["donnees"]` key, consistent with `_gabarit_id()`. Typecheck/lint green, commits verified in git log.
...
- [x] **Kubernetes : unifier les pools en une seule source de vérité (2026-09-15) — correctif vérifié, exécution test limitée par deux blocages infrastructure.** Les pools créés **avec** le cluster vivaient uniquement dans `ClusterK8s.pools` (dépôt `depot_cluster`), tandis que les pools ajoutés après via `POST /kubernetes/{id}/pools` vivaient dans `depot_pool` — deux sources de vérité disconnectées. Conséquences : (1) `PATCH`/`DELETE /kubernetes/{id}/pools/{nom}` retournaient `404` pour les pools initiaux (confirmé en direct contre `paas-shared-cluster2`), (2) les pools ajoutés disparaissaient après rechargement. **Solution** : tous les pools passent désormais par `depot_pool` comme source unique. À la création du cluster, `ExecuteurK8sCreate.terminer` migre les pools initiaux vers `depot_pool` et vide `ClusterK8s.pools`. À la lecture, une nouvelle fonction `_assembler_pools_cluster()` reconstitue la liste depuis `depot_pool`. Commit `7e597b5` sur `dev01-real-infra`. **Vérification exécution (2026-09-15, après augmentation mémoire ctrl1 28GB→40GB) : pytest lancé sur 11 tests, 2 passent, 9 échouent sur deux blocages infrastructure distincts — le correctif lui-même n'est pas cause d'échec.** Blocage 1 : `_espace_demo()` échoue avec `409 No more IP addresses available on network bd8a8421-...` (Neutron encore exhausted malgré le nettoyage de 29 routeurs/38 ports du 2026-09-15, 10h) — cause probable : nouveaux tests créant de nouvelles fuite réseau pendant cette session même. Blocage 2 : Magnum cluster name `409 Conflict` sur `POST /v1/clusters` (le cluster `capi-fix-verify` de test jamais nettoyé existe toujours, conflit avec le nom fixe du fixture). 3ème signal : tests avec monkeypatch `CREATE_IN_PROGRESS` échouent car le statut devient `degraded` au lieu du `provisioning` attendu — probable issue dans `_mapper_statut_magnum` avec ce chemin spécifique, à creuser si les deux blocages ci-dessus sont résolus. **Vérification code** : correctif sain (pools migrés + assemblage), `ruff check` vert. Le correctif reste viable mais l'exécution réelle est bloquée par l'infra du lab, pas par le code.
