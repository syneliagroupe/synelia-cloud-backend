...
| Docker extra | temporal + openstack in image | Root extras forward to `synelia[temporal]` and `synelia[openstack]`; `uv sync --frozen --no-dev --extra temporal --extra openstack`. Credentials remain env-only on the guest. |
